...
static void min_all_kernel_impl(Tensor& result, const Tensor& input) {
  if (input.scalar_type() == ScalarType::Bool) {
    ...
    AT_DISPATCH_ALL_TYPES_AND3(kHalf, kBFloat16, kFloat8, input.scalar_type(), "min_all", [&] {
      // TODO: replace vec_scalar_t with at::opmath_type when float16 specialization is done.
      using Vec = Vectorized<vec_scalar_t<scalar_t>>;
...
static void max_all_kernel_impl(Tensor& result, const Tensor& input) {
  if (input.scalar_type() == ScalarType::Bool) {
    ...
    AT_DISPATCH_ALL_TYPES_AND3(kHalf, kBFloat16, kFloat8, input.scalar_type(), "max_all", [&] {
      // TODO: replace vec_scalar_t with at::opmath_type when float16 specialization is done.
      using Vec = Vectorized<vec_scalar_t<scalar_t>>;
